{foreach $product_features as $feature}
    {if $feature.feature_type != "ProductFeatures::GROUP"|enum}
        ...
        <div class="ty-product-feature__label"><span>{$feature.description nofilter}</span>{if $feature.full_description|trim}{include file="common/popupbox.tpl" link_meta="ty-icon ty-icon-help-circle cm-dialog-auto-size" id="ut2_features_dialog_`$feature.feature_id`" text=$feature.description content=$feature.full_description show_brackets=false}{/if}</div>
        ...
        <div class="ty-product-feature__value">
            ...
            {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                ...
                {foreach from=$feature.variants item="var" name="fvars"}
                    ...
                    {if $var.selected}<li class="ty-product-feature__multiple-item">{include file="views/products/components/ab__similar_filter.tpl" feature=$feature variant_id=$var.variant_id}{if !$hide_variant_affix}<span class="ty-product-feature__prefix">{$feature.prefix}</span>{/if}
                        ...
                        {if !$hide_variant_affix}{if !$smarty.foreach.fvars.last}{if !$ab__search_similar_in_category},{/if}{/if}<span class="ty-product-feature__suffix">{$feature.suffix}</span>{/if}</li>{/if}
                {/foreach}
                </ul>
            {elseif in_array($feature.feature_type, ["ProductFeatures::TEXT_SELECTBOX"|enum, "ProductFeatures::EXTENDED"|enum, "ProductFeatures::NUMBER_SELECTBOX"|enum])}
                {foreach $feature.variants as $variant}
                    {if $variant.selected}
                        {$filter_value=$variant.variant_id}
                        {if $feature.filter_style=="slider"}
                            {$filter_value="`$variant.value_int|intval`-`$variant.value_int|intval`"}
                        {/if}
                        {include file="views/products/components/ab__similar_filter.tpl" feature=$feature variant_id=$filter_value}
                        {if $feature.filter_style == "ProductFilterStyles::COLOR"|enum && $variant.color}<div class="abt__ut2_color_mark" style="background-color: {$variant.color};width:15px;height:15px;display:inline-block;margin-right:5px;margin-top:-2px;border-radius: 50%;{if $var.color == "#ffffff"}border: 1px solid{/if}"></div>{/if}
                        {hook name="abt__ut2_features:variant"}{$variant.variant}{/hook}
                    {break}
                    {/if}
                {/foreach}
