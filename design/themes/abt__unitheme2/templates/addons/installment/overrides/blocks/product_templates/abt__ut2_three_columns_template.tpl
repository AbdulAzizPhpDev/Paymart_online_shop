                    ...
                    {if !$no_images}
                        ...
                        <div class="ut2-pb__img cm-reload-{$product.product_id}" data-ca-previewer="true" id="product_images_{$product.product_id}_update">
							{if $settings.abt__device == "mobile"}
	                            {include file="views/products/components/product_images.tpl" product=$product show_detailed_link="Y" image_width="288" image_height="288" lazy_load=false}
	                        <!--product_images_{$product.product_id}_update--></div>
                        	{else}
	                        	{include file="views/products/components/product_images.tpl" product=$product show_detailed_link="Y" image_width=$settings.Thumbnails.product_details_thumbnail_width image_height=$settings.Thumbnails.product_details_thumbnail_height lazy_load=false}
	                        <!--product_images_{$product.product_id}_update--></div>
                        	{/if}
                    {/if}
                {/hook}
...
{*                                        <label class="ty-product-options__radio--label" for="3">3 {__('month')|lower}</label>*}
    		            ...
    		            {if $settings.abt__ut2.products.view.show_short_description[$settings.abt__device] == 'Y'}
                            {if $show_short_descr && $product.short_description}
                            ...
                            {elseif !$product.header_features|count > 3 || !$product.variation_features_variants}
                                <div class="ut2-pb__short-descr" {live_edit name="product:full_description:{$product.product_id}" phrase=$product.full_description}>{$product.full_description|strip_tags|truncate:300 nofilter}</div>
                            {/if}
                        {/if}
					...
					<div class="col-right">{* Start col *}
		                ...
		                {if $capture_buttons}{capture name="buttons"}{/if}
                            ...
                            {if $settings.abt__device != "mobile"}
                                <div data-currency-name="{$smarty.const.CART_LANGUAGE}" class="installment-periods ty-control-group">
{*                                    <input type="radio" name="period" value="3" id="3" class="ty-product-options__radio">*}
{*                                    <label class="ty-product-options__radio--label" for="3">3 {__('month')|lower}</label>*}
