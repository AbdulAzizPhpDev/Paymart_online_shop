...
{hook name="products:product_name"}
    {if $show_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product nofilter}{if $hide_links}</strong>{else}{if $show_labels_in_title && $settings.abt__ut2.product_list.show_rating == "YesNo::NO"|enum}{hook name="products:video_gallery"}{/hook}{hook name="products:dotd_product_label"}{/hook}{/if}</a>{/if}
    {elseif $show_trunc_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:160:"...":true nofilter}{if $hide_links}</strong>{else}{if $show_labels_in_title && $settings.abt__ut2.product_list.show_rating == "YesNo::NO"|enum}{hook name="products:video_gallery"}{/hook}{hook name="products:dotd_product_label"}{/hook}{/if}</a>{/if}
    {/if}
{/hook}
{/capture}
{if $no_capture}
        ...
        {hook name="products:out_of_stock_block"}
            ...
            {if !$details_page}
                {if (!$product.hide_stock_info && !(($product_amount <= 0 || $product_amount < $product.min_qty) && ($product.avail_since > $smarty.const.TIME)))}
                    <button disabled class="ty-btn ty-btn__tertiary" id="out_of_stock_info_{$obj_prefix}{$obj_id}"><span><i class="ut2-icon-outline-cart"></i><span>{$out_of_stock_text}</span></span></button>
                {/if}
