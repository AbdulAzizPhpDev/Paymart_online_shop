{if $products}
	...
	{$show_labels_in_title = $settings.abt__ut2.product_list.show_rating == "YesNo::NO"|enum}
    {$defaul_show_brand_logo = $settings.abt__ut2.product_list.products_without_options.show_brand_logo[$settings.abt__device] == "YesNo::YES"|enum}
    ...
    {foreach from=$products item=product key=key name="products"}
        ...
        {hook name="products:product_block"}
            ...
            {include file="common/product_data.tpl" product=$product min_qty=true product_labels_position="left-top" show_labels_in_title=$show_labels_in_title}

            <div class="ty-product-list {if $settings.abt__ut2.product_list.decolorate_out_of_stock_products == "YesNo::YES"|enum && $product.amount < 1 && $product.out_of_stock_actions != "OutOfStockActions::BUY_IN_ADVANCE"|enum}decolorize{/if} clearfix"{if $ut2_load_more && $smarty.foreach.products.first} data-ut2-load-more="first-item"{/if}>
                ...
                <div class="ut2-pl__image" {if $settings.abt__device == "desktop"}style="width: {$settings.Thumbnails.product_lists_thumbnail_height|intval}px;"{/if}>
                    {assign var="product_link" value="products.view?product_id=`$product.product_id`"|fn_url}
                    {assign var="product_labels" value="product_labels_`$obj_prefix``$obj_id`"}
                    {$smarty.capture.$product_labels nofilter}

                    <div class="ut2-pl__list-buttons" id="{$smarty.capture.abt__service_buttons_id}">
                    ...
                    <div class="ut2-pl__info" style="min-height: {$settings.Thumbnails.product_lists_thumbnail_height - 30}px">
                        ...
                        <div class="ut2-pl__extra-block clearfix">
							...
							{hook name="products:product_rating"}
                            	<div class="ut2-pl__rating ut2-rating-stars {if $settings.abt__ut2.product_list.show_rating == "YesNo::YES"|enum && $addons.product_reviews.status == "ObjectStatuses::ACTIVE"|enum}r-block{/if}">
                                      
                                    {if !$show_labels_in_title}
