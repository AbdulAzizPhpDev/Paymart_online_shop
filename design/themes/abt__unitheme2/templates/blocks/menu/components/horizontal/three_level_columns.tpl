    ...
    <div class="ty-menu__submenu-col" {if $settings.abt__device != "mobile"}style="width:{$col_width}%"{/if}>
        {foreach from=$row item="item2" name="item2"}
            <div class="second-lvl" data-elem-index="{$second_level_counter}">
                ...
                <div class="ty-menu__submenu-item-header{if $item2.abt__ut2_mwi__icon} ut2-mwi-icon-wrap{/if}{if $item2.active || $item2|fn_check_is_active_menu_item:$block.type} ty-menu__submenu-item-header-active{/if}">
                    <a href="{$item2_url|default:"javascript:void(0)"}"{if $item2_url && $item1.new_window == "YesNo::YES"|enum} target="_blank"{/if} class="ty-menu__submenu-link{if empty($item2.$childs)} no-items{/if}{if $item2.class} {$item2.class}{/if}">{if $item2.abt__ut2_mwi__icon && $block.properties.abt_menu_icon_items == "YesNo::YES"|enum && $settings.abt__device != 'mobile'}<div class="ut2-mwi-icon">{include file="common/image.tpl" images=$item2.abt__ut2_mwi__icon class="ut2-mwi-icon" no_ids=true}</div>{/if}
                        {strip}<bdi>{$item2.$name nofilter}
                ...
                {if !empty($item2.$childs)}
                    ...
                    <div class="ty-menu__submenu{if $item1.abt__ut2_mwi__dropdown == "YesNo::YES"|enum && $item2.abt__ut2_mwi__text && $item2.abt__ut2_mwi__text_position !="bottom"} tree-level-img{/if}" {if $item1.abt__ut2_mwi__dropdown =="YesNo::YES"|enum}style="min-height: {$block.properties.abt__ut2_menu_min_height}px"{/if}>
                        {if $item1.abt__ut2_mwi__dropdown == "YesNo::YES"|enum}
                            {if $settings.abt__device != "mobile"}<div class="sub-title-two-level"><bdi>{$item2.$name nofilter}</bdi></div>{/if}
                            ...
                            {/foreach}
                        {else}
                            {if $item2.$childs}<div class="ty-menu__submenu-list {if $item1.abt__ut2_mwi__dropdown == "YesNo::YES"|enum}tree-level-col {else}{if $item2.$childs|count > $Viewlimit}hiddenCol {/if}{/if}cm-responsive-menu-submenu" {if $item2.$childs|count > $Viewlimit && $item1.abt__ut2_mwi__dropdown !="YesNo::YES"|enum}style="height: {$Viewlimit * 21}px;"{/if}>
                                {if $item1.abt__ut2_mwi__dropdown == "YesNo::YES"|enum}<div class="sub-title-two-level"><bdi>{$item2.$name nofilter}</bdi></div>{/if}
                                {hook name="blocks:topmenu_dropdown_3levels_col_elements"}
...
{if $item1.abt__ut2_mwi__status == "YesNo::YES"|enum && $item1.abt__ut2_mwi__text|trim && $item1.abt__ut2_mwi__dropdown !="YesNo::YES"|enum && $settings.abt__device != 'mobile'}
    <div class="ut2-mwi-html{if $item1.abt__ut2_mwi__dropdown == "YesNo::YES"|enum} bottom{else} {$item1.abt__ut2_mwi__text_position}{/if} hidden-phone">{$item1.abt__ut2_mwi__text nofilter}</div>
{/if}
