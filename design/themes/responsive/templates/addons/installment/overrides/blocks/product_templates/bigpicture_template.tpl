...
<div class="ut2-pb ty-product-block ty-product-detail ut2-big-image{if $settings.abt__ut2.products.view.show_sticky_add_to_cart[$settings.abt__device] == 'Y' && !in_array($product.zero_price_action, ["P","A"]) && $product.price > 0 && $product.amount > 0 && $is_add_to_cart_mv} sticky_add_to_cart{/if}">
    ...
    {if !$hide_title}
        <h1 class="ut2-pb__title" {live_edit name="product:product:{$product.product_id}"}>
    ...
    <div class="ut2-pb__wrapper clearfix">
        ...
        <div class="ut2-pb__img-wrapper ty-product-block__img-wrapper">
            ...
            {if !$no_images}
            <div class="ut2-pb__img {if $product.image_pairs|@count < 1} ut2-pb__no-thumbs{/if} cm-reload-{$product.product_id} {if $settings.Appearance.thumbnails_gallery == "YesNo::YES"|enum}ut2-pb__as-gallery{else}ut2-pb__as-thumbs{/if}"
            ...
            {else}
            {include file="views/products/components/product_images.tpl" image_width=$product_details_big_thumbnail_width image_height=$product_details_big_thumbnail_height  product=$product show_detailed_link="YesNo::YES"|enum thumbnails_size=50 }
            <!--product_images_{$product.product_id}_update--></div>
        {/if}
    {/if}
        {/hook}
    ...
    <div class="ut2-pb__right">
                    ...
                    <div class="ut2-pb__rating">
                        <div class="ty-discussion__rating-wrapper" id="average_rating_product">
                            ...
                            {$smarty.capture.$rating nofilter}<a
                                    class="ty-discussion__review-a cm-external-click"{if $add_scroll} data-ca-scroll="discussion"{/if}
                                    data-ca-external-click-id="discussion">{$product.discussion.search.total_items} {__("reviews", [$product.discussion.search.total_items])}</a>{if $discussion.type !== "Addons\\Discussion\\DiscussionTypes::TYPE_DISABLED"|enum && !$discussion.disable_adding}{include file="addons/discussion/views/discussion/components/new_post_button.tpl" name=__("write_review") obj_id=$obj_id obj_prefix="main_info_title_" style="text" object_type="Addons\\Discussion\\DiscussionObjectTypes::PRODUCT"|enum locate_to_review_tab=true}{/if}
                        </div>
                    </div>
                {elseif $product.discussion_type && $product.discussion_type != 'D'}
                    <div class="ut2-pb__rating">
                        <div class="ty-discussion__rating-wrapper">
                            <span class="ty-nowrap no-rating"><i class="ty-icon-star-empty"></i><i
                                        class="ty-icon-star-empty"></i><i class="ty-icon-star-empty"></i><i
                                        class="ty-icon-star-empty"></i><i class="ty-icon-star-empty"></i></span>
                            {if $discussion.type !== "Addons\\Discussion\\DiscussionTypes::TYPE_DISABLED"|enum && !$discussion.disable_adding}{include file="addons/discussion/views/discussion/components/new_post_button.tpl" name=__("write_review") obj_id=$obj_id obj_prefix="main_info_title_" style="text" object_type="Addons\\Discussion\\DiscussionObjectTypes::PRODUCT"|enum locate_to_review_tab=true}{/if}
                        </div>
                    </div>
                {/if}
        ...
        {*        {hook name="products:ab__motivation_block"}*}

        {*            <h1 style="color: red" > hook name="products:ab__motivation_block"</h1>*}
        {*        {/hook}*}
    ...
    {if $settings.abt__device == "mobile" || $settings.abt__device == "tablet"}
        {hook name="products:buy_together"}
            <h1 style="color: red"> hook name="products:buy_together"</h1>
        {/hook}
        {hook name="products:product_tabs_pre"}
...
{hook name="products:bottom_product_layer"}
    <h1 style="color: red">hook name="products:bottom_product_layer"</h1>
{/hook}
