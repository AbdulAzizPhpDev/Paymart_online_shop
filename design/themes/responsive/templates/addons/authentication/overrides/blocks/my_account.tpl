...
{*{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}"*}
{*<a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}*}
{*{$config.current_url|fn_url}*}
{*{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}"*}
{*   data-ca-target-id="login_block{$block.snapping_id}"*}
{*   class="cm-dialog-opener cm-dialog-auto-size " rel="nofollow" style="font-size: 18px;font-weight: bold;color: #FFFFFF">*}
{*    <i class="ty-icon-user"></i>&nbsp;*}
{*    <span class="ty-account-info__title-txt" {live_edit name="block:name:{$block.block_id}"}>*}
{*        {__("sign_in")}*}
{*    </span>*}
{*</a>*}
