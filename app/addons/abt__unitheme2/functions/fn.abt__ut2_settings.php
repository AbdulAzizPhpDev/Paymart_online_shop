...
function fn_abt__ut2_get_storefront_id()
{
...
$db_temp = db_get_array('SELECT * FROM ?:abt__ut2_settings WHERE lang_code = ?s AND storefront_id = ?i', $lang_code, fn_abt__ut2_get_storefront_id());
}
if (!empty($db_temp)) {
...
{
$lang_code = DESCR_SL;
$storefront_id = fn_abt__ut2_get_storefront_id();
...
$name = $k . '.' . $sub_k;
fn_update_abt__ut2_setting($type, $section, $name, $storefront_id, $lang_code, $sub_item, $data[$section][$name], $style);
}
} else {
$name = $k;
fn_update_abt__ut2_setting($type, $section, $name, $storefront_id, $lang_code, $item, $data[$section][$name], $style);
}
}
}
return true;
}
function fn_update_abt__ut2_setting($type, $section, $name, $storefront_id, $lang_code, $item, $v, $style)
{
$v = serialize($v);
$d = [
'section' => $section,
...
WHERE `section` = ?s AND `name` = ?s AND storefront_id = ?i', $section, $name, fn_abt__ut2_get_storefront_id());
if (empty($n)) {
...
db_query('UPDATE ?:abt__ut2_settings SET value = ?s WHERE `section` = ?s AND `name` = ?s AND storefront_id = ?i', $v, $section, $name, fn_abt__ut2_get_storefront_id());
}
}
return true;
}
