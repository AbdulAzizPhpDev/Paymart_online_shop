...
$video_types = [
'abt__ut2_background_mp4_video' => [
'device' => 'all',
],
'abt__ut2_mobile_background_mp4_video' => [
'device' => 'mobile',
],
'abt__ut2_tablet_background_mp4_video' => [
'device' => 'tablet',
],
];
foreach ($video_types as $video_type => $data) {
if (!empty($temporal[$video_type])) {
$name = $temporal['banner_id'] . '-' . $video_type . '-banner.' . strtolower(pathinfo($temporal[$video_type], PATHINFO_EXTENSION));
$video_path = fn_abt__ut2_get_video_absolute_path($temporal[$video_type]);
fn_copy($video_path, "{$path}{$name}");
$temporal[$video_type] = 'ut2_banner_videos/' . $name;
}
}
...
$notif .= '<a target="_blank" href="' . fn_url("banners.update?banner_id=$id") . '">' . $id . '</a>,';
...
WHERE lang_code=?s AND param_id = ?i', 'ru', $item['param_id']);
